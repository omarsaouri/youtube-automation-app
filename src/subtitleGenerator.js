...
function splitTextIntoSubtitles(text, maxCharsPerLine = 50, maxLinesPerSubtitle = 2) {
    ...
    for (const sentence of sentences) {
        ...
        if (currentSubtitle && (currentLineCount >= maxLinesPerSubtitle || 
            (currentSubtitle + ' ' + trimmedSentence).length > maxCharsPerLine * maxLinesPerSubtitle)) {
...
export async function generateSubtitles(storyPath, audioDuration) {
    try {
        ...
        const subtitleDuration = audioDuration / subtitleChunks.length;
        ...
        // Save SRT file
        const timestamp = path.basename(storyPath).replace('story-', '').replace('.txt', '');
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    const storyPath = path.join(__dirname, '../output/stories', process.argv[2] || 'story-2025-04-28T19-06-13-415Z.txt');
