...
import { runAutomation, getUploadStats, cleanupOldFiles } from './automationRunner.js';
...
const logger = winston.createLogger({
    ...
    transports: [
        new winston.transports.File({ filename: path.join(__dirname, '../logs/cron-error.log'), level: 'error' }),
        new winston.transports.File({ filename: path.join(__dirname, '../logs/cron-combined.log') }),
...
async function runAutomationWithRetry(maxRetries = 3) {
    ...
    while (attempt <= maxRetries) {
        try {
            ...
            if (result.success) {
                logger.info(`✅ Automation completed successfully on attempt ${attempt}`, {
...
export function displaySchedule() {
    ...
    schedules.forEach((schedule, index) => {
        const nextRun = cron.getNextDate(schedule);
        console.log(`${index + 1}. ${schedule} (Next: ${nextRun.toLocaleString()})`);
    });
...
function displayStats() {
    ...
    if (stats.lastUpload) {
        ...
        console.log(`Time: ${new Date(stats.lastUpload.timestamp).toLocaleString()}`);
    }
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('  CRON_SCHEDULE_TYPE - Schedule type (sixPerDay, threePerDay, fourPerDay, eightPerDay)');
